...
void lifetime_print(struct sadb_ext *ext)
{
	...
	printf(" %s lifetime:\n",
		   life->sadb_lifetime_exttype == SADB_EXT_LIFETIME_CURRENT ? "Current" : life->sadb_lifetime_exttype == SADB_EXT_LIFETIME_HARD ? "Hard"
																																		: "Soft");
...
void address_print(struct sadb_ext *ext)
{
	...
	printf(" %s address: ",
		   addr->sadb_address_exttype == SADB_EXT_ADDRESS_SRC ? "Source" : addr->sadb_address_exttype == SADB_EXT_ADDRESS_DST ? "Dest"
																															  : "Proxy");
